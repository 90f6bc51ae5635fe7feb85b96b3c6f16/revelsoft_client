...
const { getPortfolioBy, deletePortfolio } = usePortfolio();
...
const headers: Headers = [
  ...
  { title: "รูป", key: "portfolio_img", sortable: false, width: 80, align: 'center' },
  { title: "รหัสพอตฟอริโอ้", key: "portfolio_id", sortable: false, width: 200, },
  { title: "ชื่อ", key: "portfolio_name", sortable: false },
  { title: "รายละเอียด", key: "portfolio_detail", sortable: false },
];
const columns: any[] = [
  { value: "portfolio_id", title: "รหัสพอตฟอริโอ้", },
  { value: "portfolio_name", title: "ชื่อ", },
  { value: "portfolio_detail", title: "รายละเอียด", },
];
...
const onDelete = (id: string) => Swal.fire({
  ...
  try {
    ...
    await deletePortfolio({ portfolio_id: id })
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    <v-card-title>พอตฟอริโอ้</v-card-title>
    <v-card-item>
      <v-row class="mb-4">
      ...
      <v-data-table-server :loading="pendingportfolios" :headers="headers" :items="portfolios?.docs ?? []"
        ...
        <template v-slot:[`item.portfolio_img`]="{ item }">
          <v-avatar>
            <v-img v-if="item.raw.portfolio_img" class="mx-auto" :src="`${publicCtx.apiBaseUrl}${item.raw.portfolio_img}`"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/portfolio/detail', query: { id: item.raw.portfolio_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/portfolio/update', query: { id: item.raw.portfolio_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.portfolio_id)">
                  <v-list-item-title>
