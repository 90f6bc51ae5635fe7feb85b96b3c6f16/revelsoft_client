      ...
      <v-row>
        ...
        <div class="mt-4 mb-4" style="overflow-x: auto;">
          <table class="table table-bordered" style="min-width: 800px" aria-describedby="title">
            ...
            <tbody>
              <tr v-for="(list, idx) in banner_lists" :key="idx">
                <td>{{ idx + 1 }}</td>
                <td class="align-middle text-center" align-self="center">
                  <v-text-field v-model="list.banner_list_name" label="คุณสมบัติ" density="compact" variant="outlined"
                    hide-details style="min-width: 350px;"></v-text-field>
                </td>
                <td class="align-middle text-center"><v-icon clickable @click="deleteRow(idx)" color="error">mdi
                    mdi-close-thick</v-icon></td>
              </tr>
            </tbody>
            <tfoot>
              <tr>
