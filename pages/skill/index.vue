...
const { getSkillBy, deleteSkill } = useSkill();
...
const headers: Headers = [
  ...
  { title: "รหัสทักษะ", key: "skill_id", sortable: false, width: 200, },
  { title: "ชื่อทักษะ", key: "skill_name", sortable: false },
];
const columns: any[] = [
  { value: "skill_id", title: "รหัสทักษะ", },
  { value: "skill_name", title: "ชื่อทักษะ", },
];
...
} = await useAsyncData('skill', () => getSkillBy({
...
<template>
  <v-card elevation="10" class="withbg mx-auto" color="grey-lighten-3" max-width="1980">
    ...
    <v-card-item>
        ...
        <v-col class="d-flex flex-wrap gap-2 align-center justify-md-end py-2" cols="12" md="8">
          <form class="d-flex flex-wrap gap-2" :onsubmit="submitSearch">
            <v-text-field v-model="search.text" label="Search..." append-inner-icon="mdi-magnify" density="compact"
              variant="outlined" hide-details style="width: 500px" @click:append-inner="refreshSkill()"></v-text-field>
          </form>
      ...
      <v-data-table-server :loading="pendingSkill" :headers="headers" :items="skill?.docs ?? []"
        :items-length="skill?.totalDocs ?? 0" :items-per-page="pagination.size" :page="pagination.page"
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item class="cursor-pointer" density="compact"
                :to="{ path: '/skill/detail', query: { id: item.raw.skill_id } }">
                <v-list-item-title>
              ...
              <template v-if="permission_edit">
                <v-list-item class="cursor-pointer" density="compact"
                  :to="{ path: '/skill/update', query: { id: item.raw.skill_id } }">
                  <v-list-item-title>
              ...
              <template v-if="permission_delete">
                <v-list-item class="cursor-pointer" density="compact" @click="onDelete(item.raw.skill_id)">
                  <v-list-item-title>
